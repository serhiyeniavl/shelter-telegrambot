...
public class UpdateObjectWrapperUtils {
    ...
    public static Pair<UserCommand, String> parseCallbackData(Update update) {
        String data = update.getCallbackQuery().getData();
        String[] split = data.split("\\s+");
        return (split.length > 1) ? Pair.of(UserCommand.fromValue(split[0]), split[1]) : Pair.of(UserCommand.fromValue(split[0]), "");
    }
}
