...
class LeaveCommandActionTest extends BaseSpringBootTestClass {
    ...
    void leaveNonExistingRoomRoom() throws TelegramApiException {
        long telegramUserId = 100L;

        User expectedUser = saveUser(telegramUserId + 1, UserActionState.NEW_USER, EN_US.toString());
        Room expectedRoom = saveRoom(telegramUserId, Set.of(106L, 102L, 103L, 104L), 4, RoomState.WAITING_TO_JOIN, LocalDateTime.now());
